...
export class BankService {
  ...
  async getBlockDetails() {
    try {
     ...
     const strBD = blockDetails.map(i=>{return i.toString()});
     console.log("blockDetails:",strBD)
      return {
        blockNumber:strBD[0],previousRand:strBD[1],
        gasLimit:strBD[2],miner:strBD[3]
      };
    } catch (error) {
      //  Todo: 에러를 응답합니다.(exceptions.createBadRequestException(error.message))
      throw exceptions.createBadRequestException(error)
    }
  }
